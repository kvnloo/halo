...
async function main() {
  ...
  const stats = await page.evaluate(() => { const c=globalThis.__HALO__.ctx?globalThis.__HALO__.ctx.config:null; return {exposure:c&&c.exposure, ev:c&&c.exposureEV, tm:c&&c.tonemapper, bi:c&&c.bloomIntensity, keys:c?Object.keys(c).filter(k=>/bloom|expos|taa/i.test(k)):[]}; });
