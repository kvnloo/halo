...
async function main() {
  ...
  for (const pose of poses) {
    const dataUrls = await page.evaluate(async (pose, settle, t, nvid, cfgJson) => {
      const H = globalThis.__HALO__;
      H.setSize(innerWidth, innerHeight);
      H.setPose(pose); try{ const CFG=JSON.parse(cfgJson); for(const k in CFG) H.setConfig(k, CFG[k]); }catch(e){}
      H.setTime(Number(t));
      H.advance(settle);
      if (nvid > 0) {
        const out = [];
      ...
      return [H.screenshot()];
    }, pose, OPT.settle, OPT.time, OPT.video, (process.env.CFG||'{}'), { timeout: OPT.timeout });
