"""
PINNED SNAPSHOT of tools/metrics.py, md5 0592ed3f, taken 2026-07-31 02:15.

Do NOT use this for new work — use tools/metrics.py. This copy exists only so the
before/after numbers in reports/posefit.md stay reproducible. During the Wave I pose
refit, tools/metrics.py was re-banded and re-weighted by a concurrent agent and its
hash changed THREE times inside one measurement pass (a580fdc9 -> 92b2b50a -> 0592ed3f),
which silently split one A/B across three different scoring functions. Pinning is the
only way to state "same instrument on both sides" and mean it.

If tools/metrics.py has since settled, re-measure against it and delete this file.
"""
#!/usr/bin/env python
"""
